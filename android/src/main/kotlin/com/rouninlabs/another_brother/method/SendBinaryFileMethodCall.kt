...
/**
 * Command for sending a binary file to a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class SendBinaryFileMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "sendBinaryFile"
    }
    ...
    fun execute() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                Log.e("SendBinaryFile", "NPE during send operation - likely SDK internal error", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                    }.toMap())
                ...
                Log.e("SendBinaryFile", "Error during send operation", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_BROTHER_PRINTER_NOT_FOUND
                    }.toMap())
