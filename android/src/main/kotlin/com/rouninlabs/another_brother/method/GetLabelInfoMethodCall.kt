 ...
 * approach.
 */
class GetLabelInfoMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetLabelInfo", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(LabelInfo())
                        }
                        return@launch
                    ...
                    Log.e("GetLabelInfo", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(LabelInfo())
                    }
                    return@launch
                } catch (e: Exception) {
                    Log.e("GetLabelInfo", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(LabelInfo())
                    }
                    return@launch
