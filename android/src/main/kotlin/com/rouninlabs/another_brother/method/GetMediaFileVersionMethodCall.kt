...
/**
 * Command for getting the media file version of a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class GetMediaFileVersionMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "getMediaFileVer"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetMediaFileVersion", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success("")
                        }
                        return@launch
                    ...
                    Log.e("GetMediaFileVersion", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
                } catch (e: Exception) {
                    Log.e("GetMediaFileVersion", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
