 ...
 * approach.
 */
class PrintFileListMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("PrintFileList", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(PrinterStatus().apply {
                    ...
                    Log.e("PrintFileList", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
                            errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                        }.toMap())
                    ...
                    Log.e("PrintFileList", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
