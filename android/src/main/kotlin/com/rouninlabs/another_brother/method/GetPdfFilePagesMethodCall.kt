 ...
 * approach.
 */
class GetPdfFilePagesMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetPdfFilePages", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(-1)
                        }
                        return@launch
                    ...
                    Log.e("GetPdfFilePages", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(-1)
                    }
                    return@launch
                } catch (e: Exception) {
                    Log.e("GetPdfFilePages", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(-1)
                    }
                    return@launch
