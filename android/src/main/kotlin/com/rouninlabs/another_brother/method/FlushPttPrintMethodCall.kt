...
/**
 * Command for sending flushPTTPrint to a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class FlushPttPrintMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "flushPTTPrint"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("FlushPttPrint", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(PrinterStatus().apply {
                    ...
                    Log.e("FlushPttPrint", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
                            errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                        }.toMap())
                    ...
                    Log.e("FlushPttPrint", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
            ...
            // Flush PTT Print - only if startCommunication succeeded
            // Wrap in try-catch to handle SDK internal errors (e.g., null OutputStream)
            try {
                val flushResult = printer.flushPTTPrint()
                ...
                val dartPrintStatus = flushResult.toMap()
                withContext(Dispatchers.Main) {
                    // Set result Printer status.
                    result.success(dartPrintStatus)
                }
            } catch (e: NullPointerException) {
                Log.e("FlushPttPrint", "NPE during print operation - likely SDK internal error", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                    }.toMap())
                ...
                Log.e("FlushPttPrint", "Error during print operation", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_BROTHER_PRINTER_NOT_FOUND
                    }.toMap())
