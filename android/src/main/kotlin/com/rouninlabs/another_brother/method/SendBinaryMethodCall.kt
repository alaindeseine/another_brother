 ...
 * approach.
 */
class SendBinaryMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("SendBinary", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(PrinterStatus().apply {
                    ...
                    Log.e("SendBinary", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
                            errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                        }.toMap())
                    ...
                    Log.e("SendBinary", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(PrinterStatus().apply {
            ...
            // Send Binary - only if startCommunication succeeded
            ...
            try {
                ...
                Log.e("SendBinary", "NPE during send operation - likely SDK internal error", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                    }.toMap())
                ...
                Log.e("SendBinary", "Error during send operation", e)
                withContext(Dispatchers.Main) {
                    result.success(PrinterStatus().apply {
                        errorCode = PrinterInfo.ErrorCode.ERROR_BROTHER_PRINTER_NOT_FOUND
                    }.toMap())
