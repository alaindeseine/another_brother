...
/**
 * Command for getting the system report of a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class GetSystemReportMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "getSystemReport"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetSystemReport", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success("")
                        }
                        return@launch
                    ...
                    Log.e("GetSystemReport", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
                } catch (e: Exception) {
                    Log.e("GetSystemReport", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
