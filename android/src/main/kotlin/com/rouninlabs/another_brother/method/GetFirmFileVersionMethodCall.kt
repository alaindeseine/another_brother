...
/**
 * Command for getting the firmware file version of a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class GetFirmFileVersionMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "getFirmFileVer"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetFirmFileVersion", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success("")
                        }
                        return@launch
                    ...
                    Log.e("GetFirmFileVersion", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
                } catch (e: Exception) {
                    Log.e("GetFirmFileVersion", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success("")
                    }
                    return@launch
            ...
            // Get Firmware File Version - only if startCommunication succeeded
            // Wrap in try-catch to handle SDK internal errors (e.g., null OutputStream)
            try {
                val firmFileVersion = printer.getFirmFileVer(filePath)
                ...
                Log.e("GetFirmFileVersion", "NPE during get operation - likely SDK internal error", e)
                withContext(Dispatchers.Main) {
                    result.success("")
                }
            } catch (e: Exception) {
                Log.e("GetFirmFileVersion", "Error during get operation", e)
                withContext(Dispatchers.Main) {
                    result.success("")
                }
            }
