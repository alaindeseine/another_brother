...
/**
 * Command for getting the bluetooth preference of a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class GetBluetoothPreferenceMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "getBluetoothPreference"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetBluetoothPreference", "Failed to start communication with printer")
                        withContext(Dispatchers.Main) {
                            result.success(hashMapOf(
                                "printerStatus" to PrinterStatus().apply {
                    ...
                    Log.e("GetBluetoothPreference", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    withContext(Dispatchers.Main) {
                        result.success(hashMapOf(
                            "printerStatus" to PrinterStatus().apply {
                                errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                    ...
                    Log.e("GetBluetoothPreference", "Error in startCommunication", e)
                    withContext(Dispatchers.Main) {
                        result.success(hashMapOf(
                            "printerStatus" to PrinterStatus().apply {
            ...
            // Get Bluetooth Preference - only if startCommunication succeeded
            // Wrap in try-catch to handle SDK internal errors (e.g., null OutputStream)
            try {
                val outPrefs = BluetoothPreference()
                val printResult = printer.getBluetoothPreference(outPrefs);
                ...
                val dartOutValues:Map<String, Any> = outPrefs.toMap();
                withContext(Dispatchers.Main) {
                    // Set result Printer status.
                    result.success(hashMapOf(
                ...
                Log.e("GetBluetoothPreference", "NPE during get operation - likely SDK internal error", e)
                withContext(Dispatchers.Main) {
                    result.success(hashMapOf(
                        "printerStatus" to PrinterStatus().apply {
                            errorCode = PrinterInfo.ErrorCode.ERROR_COMMUNICATION_ERROR
                ...
                Log.e("GetBluetoothPreference", "Error during get operation", e)
                withContext(Dispatchers.Main) {
                    result.success(hashMapOf(
                        "printerStatus" to PrinterStatus().apply {
                            errorCode = PrinterInfo.ErrorCode.ERROR_BROTHER_PRINTER_NOT_FOUND
