...
/**
 * Command for getting the boot mode value of a Brother printer.
 * This support both one-time as well as the standard openCommunication/print/closeCommunication
 * approach.
 */
class GetBootModeMethodCall(val flutterAssets: FlutterPlugin.FlutterAssets, val context: Context, val call: MethodCall, val result: MethodChannel.Result) {
    companion object {
        const val METHOD_NAME = "getBootMode"
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (isOneTime) {
                try {
                    ...
                    if (!started) {
                        Log.w("GetBootMode", "Failed to start communication with printer")
                    }
                } catch (e: NullPointerException) {
                    Log.e("GetBootMode", "NPE in startCommunication - Bluetooth/WiFi connection failed", e)
                    // Continue anyway - the comment suggests operation may work without startCommunication
                } catch (e: Exception) {
                    Log.e("GetBootMode", "Error in startCommunication", e)
                    // Continue anyway - the comment suggests operation may work without startCommunication
                }
            }
